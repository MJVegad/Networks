  ...
  while(1)
  {
    ...
    if((clientid=accept(sockid,(struct sockaddr*)&client_addr,&client_addr_len))<0)
    ...
    else
     {
       ...
       //to start a new thread to handle client request
       if(pthread_create(&mythread,0,doProcessing,&clientid)!=0)
